...
class PaginacaoFRONT {
    ...
    public function __construct($itens,$limiteDeItens = 1)
    {
        $this->limiteDeItens=$limiteDeItens;
        $this->paginaAtual=(!empty($_GET['pagina']) ? $_GET['pagina'] : '1');
        $this->totalDeProdutos = count($itens);
    ...
    {   
        $query=str_replace("&pagina={$this->paginaAtual}",'',$_SERVER['QUERY_STRING']);
        
        $host= $_SERVER['HTTP_HOST'] . $_SERVER['PATH_INFO'] .'?'. $query;
